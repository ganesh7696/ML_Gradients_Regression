...
function stochastic_Gradient(standarizedX,X_train, X_test, Y_train, Y_test,regularization)
...
while(err_threshold == 0 || err_threshold>=2264782890.6147 && count<=20)
    lambda = 0.0002; % Learning rate to find the global minima
    Y_hat = zeros(m,1);
    p = randperm(m); % Random numbers generation in range  of training set size
    p = p(1:m);
    for j = 1: m
        l = p(j);
        Y_hat(l) = dot(w,X_train(l,:)); % Finding y_hat using dot product of weight and training data instance
