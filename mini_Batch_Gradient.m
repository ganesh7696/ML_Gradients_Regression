...
function mini_Batch_Gradient(standarizedX,X_train, X_test, Y_train, Y_test,regularization)
...
alpha = 0.5; %penalty coefficient to minimize overfitting
count =0;
% For next step assigned  err_threshold to be 2369488008.564bound on running 20epochs to verify error minimum
while(err_threshold == 0 || err_threshold>=2269488008.4063 && count<=20)
    Y_hat = zeros(m,1);
    idx = linspace(1,m,3);
    parition = idx(2);
    batch_1 = X_train((1:parition),:);
    for j = 1: length(batch_1)
    ...
    elseif regularization == "l2"
        ...
        w = w - (lambda * gradient_MSE');
        min_idx =find(w == min(w));
        w(min_idx) =0;
    else
        gradient_MSE = 2/m * (X_train)'*((X_train * w') - Y_train); % Finding gradient(derivative) of MSE
        % For next step assigned  err_threshold to be 2264782890.6779 bound on running 10epochs to verify error minimum
        w = w - (lambda * gradient_MSE');
    end
    % Updating weight vector based on gradientMSE after  batch processed
    for j = parition+1: m
        Y_hat(j) = dot(w,X_train(j,:)); % Finding y_hat using dot product of weight and training data instance
    end
    %Updating weight vector based on gradientMSE after  batch processed
